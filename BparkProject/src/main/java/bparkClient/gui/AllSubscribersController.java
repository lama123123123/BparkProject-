...
/**
 * Controller for AllSubscribers.fxml.
...
public class AllSubscribersController {
        ...
        subscribersTable.setRowFactory(tv -> {
            ...
            row.setOnMouseClicked(event -> {
                if (event.getClickCount() == 2 && (!row.isEmpty())) {
                    ...
                    bparkClient.logic.AppSessionData.getInstance().setDetailsOrigin("usher");
                    try {
    ...
    private void handleBack() {
        try {
            javafx.fxml.FXMLLoader loader = new javafx.fxml.FXMLLoader(getClass().getResource("/bparkClient/fxml/UsherMainPage.fxml"));
            javafx.scene.Parent root = loader.load();
